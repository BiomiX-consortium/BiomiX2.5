  ...
  tagList(
    ...
    div(
      ...
      fluidRow(
        ...
        column(3,
          .adv_param_block("N° top features",
            selectInput(ns("biblio_n_features"), NULL,
      ...
      fluidRow(
        ...
        column(6,
          .adv_param_block("Clinical data file (optional)",
            div(style = "display:flex; gap:10px; align-items:center;",
              shinyFilesButton(ns("clinic_file_btn"), "Browse...",
                               "Select clinical data file",
                               multiple = FALSE, class = "btn-browse-sm"),
              uiOutput(ns("clinic_file_label"))
            )
          )
        )
      )
...
advMofaServer <- function(id, roots) {
  moduleServer(id, function(input, output, session) {
    ...
    clinic_file_path  <- reactiveVal(EMPTY_VALUE)
    diablo_design_path <- reactiveVal(EMPTY_VALUE)

    # Clinical file
    shinyFileChoose(input, "clinic_file_btn", roots = roots, session = session,
                    filetypes = c("tsv","csv","txt","xlsx","xls"))
    observeEvent(input$clinic_file_btn, {
      info <- parseFilePaths(roots, input$clinic_file_btn)
      if (nrow(info) > 0) clinic_file_path(as.character(info$datapath[1]))
    }, ignoreNULL = TRUE, ignoreInit = TRUE)

    output$clinic_file_label <- renderUI({
      p <- clinic_file_path()
    ...
    reactive({
      list(
        ...
        biblio_n_features  = input$biblio_n_features  %||% "100",
