...
advSnfNemoUI <- function(id) {
  ...
  tagList(
    ...
    fluidRow(
      column(3,
        .adv_param_block("N° neighbors",
          numericInput(ns("snf_neighbors"), NULL, value = 10, min = 2, step = 1, width = "100%")
        )
      ),
      column(3,
    ...
    fluidRow(
      ...
      column(3,
        .adv_param_block("Variance affinity matrix",
          numericInput(ns("nemo_variance"), NULL, value = 0.5, min = 0.01, max = 1, step = 0.05, width = "100%")
        )
      )
    ),
    ...
    fluidRow(
      column(4,
        .adv_param_block("Numerical metadata columns",
          selectInput(ns("snf_num_cols"), NULL,
                      choices = NULL, multiple = TRUE, width = "100%")
        )
      ),
      column(4,
        .adv_param_block("Categorical metadata columns",
          selectInput(ns("snf_cat_cols"), NULL,
                      choices = NULL, multiple = TRUE, width = "100%")
        )
      ),
      column(4,
    ...
    fluidRow(
      column(3,
        .adv_param_block("Max cluster range",
          numericInput(ns("snf_cluster_range"), NULL, value = 8, min = 2, step = 1, width = "100%")
        )
      ),
      column(3,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      if (!is.null(df)) {
        ...
        updateSelectInput(session, "snf_num_cols",      choices = num_cols, selected = NULL)
        updateSelectInput(session, "snf_cat_cols",      choices = cat_cols, selected = NULL)
    ...
    reactive({
      ...
      num_cols <- paste(input$snf_num_cols %||% character(0), collapse = "/")
      cat_cols <- paste(input$snf_cat_cols %||% character(0), collapse = "/")
      ...
      list(
        ...
        nemo_variance     = input$nemo_variance     %||% 0.5,
        snf_num_cols      = if (nchar(num_cols) > 0) num_cols else EMPTY_VALUE,
        snf_cat_cols      = if (nchar(cat_cols) > 0) cat_cols else EMPTY_VALUE,
