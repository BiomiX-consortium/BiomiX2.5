...
def get_rscript_path(script_dir):
    """Return the path to Rscript: bundled R on Windows, conda env on macOS/Linux."""
    if sys.platform == "win32":
        return os.path.join(script_dir, "_INSTALL", "R_BiomiX", "bin", "Rscript.exe")
    conda_prefix = os.environ.get("CONDA_PREFIX", "")
    return os.path.join(conda_prefix, "bin", "Rscript") if conda_prefix else "Rscript"
...
"}")
        ...
        block3_middle_layout.addWidget(self.radioButton_4)
        ...
        block3_middle_layout.addWidget(self.radioButton_5)
        ...
        block3_middle_layout.addWidget(self.radioButton_6)
        ...
        # Add first set of radio buttons to group1 (YES/NO)
        self.group1.addButton(self.radioButton)
        self.group1.addButton(self.radioButton_2)

        # Add second set of radio buttons to group2 (MOFA/DIABLO)
...
"}")
        ...
        self.pushButton.clicked.connect(self.radio_selected_integration)
    ...
    def retranslateUi(self, BiomiX):
        ...
        self.label_40.setText(_translate("BiomiX", "         Filter_samples_by_Label  "))
        ...
        self.radioButton_4.setText(_translate("BiomiX", "MOFA"))
...
#READ THE METADATA TO UPLOAD ALL THE CONDITIONS AVAILABLES
    ...
    def open_shiny_app(self):
        ...
        rscript_path = get_rscript_path(script_dir)
        # Define the path to the R script
        r_script_path = os.path.join(script_dir, "BiomiX_preview_Imputation.r")
        # Command to run the R script using the specific R version
        command = [rscript_path, r_script_path]
        # Update GUI label
...
#UPLOAD OMICS FILES FUNCTION
    ...
    # def radio_selected_integration(self):
    #     if self.radioButton_4.isChecked():
    #         radio = "MOFA_INTEGRATION"
    #     if self.radioButton_5.isChecked():
    #         radio = "DIABLO_INTEGRATION"
    #     input_mofa.append(radio)

    def radio_selected_integration(self):
        radio = None  # default to avoid UnboundLocalError
        if self.radioButton_4.isChecked():
            radio = "MOFA_INTEGRATION"
        elif self.radioButton_5.isChecked():  # use elif if mutually exclusive
            radio = "DIABLO_INTEGRATION"
        ...
        if radio:  # only append if radio is assigned
            ...
            print("⚠️ No integration method button selected.")
            radio = "NO_INTEGRATION"
            input_mofa.append(radio)
...
#OPENING 2° WINDOWS        
    ...
    def show_second_window(self):
        ...
        self.second_window.push_design_diablo.clicked.connect(self.open_dialog_design_diablo)
    ...
    def save_advanced_options(self):
        ...
        N_neighbors_SNF = self.second_window.spinBox_49.value()
        Variance_affinity_matrix = self.second_window.doubleSpinBox_69.value()
        N_iteration_SNF = self.second_window.spinBox_50.value()

        SNF_options.append(N_neighbors_SNF)
        SNF_options.append(Variance_affinity_matrix)
        SNF_options.append(N_iteration_SNF)
        
        N_neighbors_NEMO = self.second_window.spinBox_51.value()
        Variance_affinity_matrix_NEMO = self.second_window.doubleSpinBox_70.value()

        NEMO_options.append(N_neighbors_NEMO)
        NEMO_options.append(Variance_affinity_matrix_NEMO)
        NEMO_options.append("X")
        
        Column_1_SNF_NEMO = [item.text() for item in self.second_window.listWidget_1.selectedItems()]
        Column_1_SNF_NEMO = "/".join(Column_1_SNF_NEMO) if Column_1_SNF_NEMO else "X"
        Column_2_SNF_NEMO = [item.text() for item in self.second_window.listWidget_2.selectedItems()]
        Column_2_SNF_NEMO = "/".join(Column_2_SNF_NEMO) if Column_2_SNF_NEMO else "X"
        Column_3_SNF_NEMO = self.second_window.comboBox_54.currentText()
        
        SNF_NEMO_metadata_variables.append(Column_1_SNF_NEMO)
        SNF_NEMO_metadata_variables.append(Column_2_SNF_NEMO)
        SNF_NEMO_metadata_variables.append(Column_3_SNF_NEMO)
        
        Max_cluster_range = self.second_window.spinBox_52.value()
        N_variable_heatmap = self.second_window.spinBox_53.value()
        N_features_max_SNF_NEMO = self.second_window.spinBox_53.value()

        SNF_NEMO_numeric_options.append(Max_cluster_range)
        SNF_NEMO_numeric_options.append(N_variable_heatmap)
        SNF_NEMO_numeric_options.append(N_features_max_SNF_NEMO)
        
        Diablo_feature_selection = self.second_window.comboBox_62_2.currentText()
        N_Iteration_diablo = self.second_window.SpinBox_59_2.value()
...
#UPLOAD OMICS FILES FUNCTION
    ...
    def open_dialog_diablo(self,file_number):
        dialog = QFileDialog()
        dialog.setFileMode(QFileDialog.ExistingFile)
        dialog.setNameFilter("Metadata Files (*.txt *.csv *.tsv *.xls *.xlsx)")
        dialog.setWindowTitle("Select Diablo design matrix data file")
        if dialog.exec_():
            file_path = dialog.selectedFiles()[0]
            file_paths_design_diablo.append(file_path)
            print(file_path)
            file_numbers_diablo.append(file_number)
            print(file_numbers_diablo)
            global global_variable7
            global_variable7 = os.path.basename(file_path)
            #print(global_variable5)
    ...
    def open_dialog_design_diablo(self):
        self.open_dialog_diablo(0)
        if global_variable7 != 0:
            self.second_window.push_design_diablo.setText(global_variable7)
            self.second_window.push_design_diablo.setStyleSheet("QPushButton { text-align: left; background-color: green; color: white }")
            self.second_window.push_design_diablo.setMaximumWidth(120)
    ...
    def table_preparation_advanced(self):
        ...
        while len(file_paths_design_diablo) < 3:
            file_paths_design_diablo.append("X")
        ...
        print(SNF_options)
        print(NEMO_options)
        print(SNF_NEMO_metadata_variables)
        ...
        df = pd.DataFrame(
            ...
            columns=[
                ...
                "ADVANCED_OPTION_SNF_OPTIONS",
                "ADVANCED_OPTION_NEMO_OPTIONS",
...
"}")
        ...
        block4_line1_2_layout = QtWidgets.QHBoxLayout()
        
        
        #inital line
        self.line_95_2 = QtWidgets.QFrame(self.tab)
        self.line_95_2.setFrameShape(QtWidgets.QFrame.Shape.HLine)
        self.line_95_2.setFrameShadow(QtWidgets.QFrame.Shadow.Sunken)
        self.line_95_2.setObjectName("line_95_2")
        block4_line1_2_layout.addWidget(self.line_95_2)
        
        #label mofa options
        self.label_312_2 = QtWidgets.QLabel(self.tab)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        font.setWeight(50)
        self.label_312_2.setFont(font)
        self.label_312_2.setObjectName("label_312_2")
        block4_line1_2_layout.addWidget(self.label_312_2)
        
        #ending line
        self.line_96_2 = QtWidgets.QFrame(self.tab)
        self.line_96_2.setFrameShape(QtWidgets.QFrame.Shape.HLine)
        self.line_96_2.setFrameShadow(QtWidgets.QFrame.Shadow.Sunken)
        self.line_96_2.setObjectName("line_96_2")
        block4_line1_2_layout.addWidget(self.line_96_2)
        
        
        
        block4_layout.addLayout(block4_line1_2_layout)
        
        self.line_95_2.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_312_2.setSizePolicy(QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.line_96_2.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        
        
        
        block4_line2_2_layout = QtWidgets.QHBoxLayout()
        
        
        # Create the label
        self.label_link_DIABLO = QtWidgets.QLabel(self)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        font.setWeight(50)
        self.label_link_DIABLO.setFont(font)
        self.label_link_DIABLO.setObjectName("label_link_DIABLO")

        # Set the HTML content with a link
        self.label_link_DIABLO.setText('<a href="https://ixi-97.github.io/Parameters_guidelines.html#MOFA_options">Help DIABLO parameters</a>')
        self.label_link_DIABLO.setOpenExternalLinks(True)  # Allows opening links in a web browser
        
        
        self.label_link_DIABLO.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        
        block4_line2_2_layout.addWidget(self.label_link_DIABLO)
        block4_layout.addLayout(block4_line2_2_layout)
        ...
        block4_line2_3_layout = QtWidgets.QHBoxLayout()

        
        
        
        #Label DESIGN MATRIX
        self.label_249_2 = QtWidgets.QLabel(self.tab)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        font.setWeight(50)
        self.label_249_2.setFont(font)
        self.label_249_2.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_249_2.setObjectName("label_249_2")
        block4_line2_3_layout.addWidget(self.label_249_2)
        
        self.push_design_diablo = QtWidgets.QPushButton(self.tab)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        font.setBold(False)
        font.setWeight(50)
        self.push_design_diablo.setFont(font)
        block4_line2_3_layout.addWidget(self.push_design_diablo)

        #Label FEATURE SELECTION DIABLO
        self.label_250_2 = QtWidgets.QLabel(self.tab)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_250_2.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_250_2.setObjectName("label_250_2")
        block4_line2_3_layout.addWidget(self.label_250_2)
        
        self.comboBox_62_2 = QtWidgets.QComboBox(self.tab)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        font.setWeight(50)
        self.comboBox_62_2.setFont(font)
        self.comboBox_62_2.setAutoFillBackground(False)
        self.comboBox_62_2.setObjectName("comboBox_62_2")
        self.comboBox_62_2.addItem("")
        self.comboBox_62_2.addItem("")
        block4_line2_3_layout.addWidget(self.comboBox_62_2)
        
        #Combobox NUMBER PCA in DIABLO
        self.label_252_2 = QtWidgets.QLabel(self.tab)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        font.setWeight(50)
        self.label_252_2.setFont(font)
        self.label_252_2.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_252_2.setObjectName("label_252_2")
        block4_line2_3_layout.addWidget(self.label_252_2)
        
        
        self.SpinBox_59_2 = QtWidgets.QSpinBox(self.tab)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        font.setWeight(50)
        self.SpinBox_59_2.setFont(font)
        self.SpinBox_59_2.setSingleStep(1)
        self.SpinBox_59_2.setProperty("value", 100)
        self.SpinBox_59_2.setObjectName("SpinBox_59_2")
        block4_line2_3_layout.addWidget(self.SpinBox_59_2)
        
        
        
        block4_layout.addLayout(block4_line2_3_layout)
        
        
        self.label_249_2.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.push_design_diablo.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_250_2.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        ...
        # Set the HTML content with a link
        self.label_link_MOFA_interpretation.setText('<a href="https://ixi-97.github.io/Parameters_guidelines.html#MOFA_interpretation">Help interpretation parameters</a>')
...
"}")
        ...
        self.label_327.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
...
"}")
        ...
        #TAB SNF_NEMO contains the SNF-NEMO options tab
        ...
        self.SNF_NEMO = QtWidgets.QWidget()
        self.SNF_NEMO.setObjectName("SNF_NEMO")


        main_layout = QtWidgets.QVBoxLayout(self.SNF_NEMO)
        ...
        #starting line
        self.line_122 = QtWidgets.QFrame(self.SNF_NEMO)
        ...
        block5_line1_layout.addWidget(self.line_122)

        #label filtering by column
        self.label_345 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        block5_line1_layout.addWidget(self.label_345)

        #ending line
        self.line_123 = QtWidgets.QFrame(self.SNF_NEMO)
        self.line_123.setGeometry(QtCore.QRect(490, 10, 261, 20))
        ...
        block5_line1_layout.addWidget(self.line_123)

        self.line_122.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_345.setSizePolicy(QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.line_123.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)

        block5_layout.addLayout(block5_line1_layout)
        
        
        block5_line2_layout = QtWidgets.QHBoxLayout()
        ...
        #Label K.SNF
        self.label_319 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_319.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_319.setObjectName("label_319")
        block5_line2_layout.addWidget(self.label_319)

        self.spinBox_49 = QtWidgets.QSpinBox(self.SNF_NEMO) #OLD 48
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.spinBox_49.setProperty("value", 10)
        self.spinBox_49.setObjectName("spinBox_49")
        block5_line2_layout.addWidget(self.spinBox_49)
        
        #Label Sigma
        self.label_320 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_320.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_320.setObjectName("label_320")
        block5_line2_layout.addWidget(self.label_320)
        
        
        self.doubleSpinBox_69 = QtWidgets.QDoubleSpinBox(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.doubleSpinBox_69.setSingleStep(0.25)
        self.doubleSpinBox_69.setProperty("value", 0.5)
        self.doubleSpinBox_69.setObjectName("doubleSpinBox_69")
        block5_line2_layout.addWidget(self.doubleSpinBox_69)
        
        
        #Label t (Number of iteration)
        self.label_321 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_321.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_321.setObjectName("label_321")
        block5_line2_layout.addWidget(self.label_321)
        
        self.spinBox_50 = QtWidgets.QSpinBox(self.SNF_NEMO) #OLD 48
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.spinBox_50.setProperty("value", 20)
        self.spinBox_50.setObjectName("spinBox_50")
        block5_line2_layout.addWidget(self.spinBox_50)
        
        self.label_319.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.spinBox_49.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_320.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.doubleSpinBox_69.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_321.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.spinBox_50.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        
        block5_layout.addLayout(block5_line2_layout)
        
        
        #LINE 3* ROW
        
        block5_line3_layout = QtWidgets.QHBoxLayout()



        #starting line
        self.line_124 = QtWidgets.QFrame(self.SNF_NEMO)
        ...
        block5_line3_layout.addWidget(self.line_124)

        #label filtering by column
        self.label_346 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        block5_line3_layout.addWidget(self.label_346)

        #ending line
        self.line_125 = QtWidgets.QFrame(self.SNF_NEMO)
        self.line_125.setGeometry(QtCore.QRect(490, 10, 261, 20))
        ...
        block5_line3_layout.addWidget(self.line_125)

        self.line_124.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_346.setSizePolicy(QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.line_125.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)

        block5_layout.addLayout(block5_line3_layout)
        
        

        
        block5_line4_layout = QtWidgets.QHBoxLayout()

        
        
        
        #NEMO PARAMETERS LINE



        #Label K.SNF
        self.label_322 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_322.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_322.setObjectName("label_322")
        block5_line4_layout.addWidget(self.label_322)

        self.spinBox_51 = QtWidgets.QSpinBox(self.SNF_NEMO) #OLD 48
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.spinBox_51.setProperty("value", 10)
        self.spinBox_51.setObjectName("spinBox_51")
        block5_line4_layout.addWidget(self.spinBox_51)
        
        #Label Sigma
        self.label_328 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_328.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_328.setObjectName("label_328")
        block5_line4_layout.addWidget(self.label_328)
        
        
        self.doubleSpinBox_70 = QtWidgets.QDoubleSpinBox(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.doubleSpinBox_70.setSingleStep(0.25)
        self.doubleSpinBox_70.setProperty("value", 0.5)
        self.doubleSpinBox_70.setObjectName("doubleSpinBox_70")
        block5_line4_layout.addWidget(self.doubleSpinBox_70)

        
        self.label_319.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.spinBox_51.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_328.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.doubleSpinBox_70.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)


        block5_layout.addLayout(block5_line4_layout)
        ...
        block5_line5_layout = QtWidgets.QHBoxLayout()



        #starting line
        self.line_126 = QtWidgets.QFrame(self.SNF_NEMO)
        ...
        block5_line5_layout.addWidget(self.line_126)

        #label filtering by column
        self.label_347 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        block5_line5_layout.addWidget(self.label_347)

        #ending line
        self.line_127 = QtWidgets.QFrame(self.SNF_NEMO)
        self.line_127.setGeometry(QtCore.QRect(490, 10, 261, 20))
        ...
        block5_line5_layout.addWidget(self.line_127)

        self.line_126.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_347.setSizePolicy(QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.line_127.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)

        block5_layout.addLayout(block5_line5_layout)
        
        
        #ROW 6 COMMON OPTIONS SNF/NEMO
        
        
        #Enrichment variable (AGE/SEX)
        
        block5_line6_layout = QtWidgets.QHBoxLayout()
        
        # label column name
        self.label_348 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_348.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_348.setObjectName("label_348")
        block5_line6_layout.addWidget(self.label_348)
        
        
        self.listWidget_1 = QtWidgets.QListWidget(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        block5_line6_layout.addWidget(self.listWidget_1)
        
        
        #SURVIVAL VARIABLES
        
        # label column name
        self.label_349 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_349.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_349.setObjectName("label_349")
        block5_line6_layout.addWidget(self.label_349)
        
        
        # combobox column name
        self.listWidget_2 = QtWidgets.QListWidget(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        block5_line6_layout.addWidget(self.listWidget_2)
        
        
        # label column name
        self.label_350 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_350.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_350.setObjectName("label_350")
        block5_line6_layout.addWidget(self.label_350)
        
        # combobox column name
        self.comboBox_54 = QtWidgets.QComboBox(self.SNF_NEMO) #Similar to 82
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.comboBox_54.setAutoFillBackground(False)
        self.comboBox_54.setObjectName("comboBox_54")
        block5_line6_layout.addWidget(self.comboBox_54)
        
        
        self.label_348.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.listWidget_1.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_349.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.listWidget_2.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_350.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.comboBox_54.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)

        block5_layout.addLayout(block5_line6_layout)
        
        
        #LINE 7 COMMON OPTION SNF/NEMO NUMERIC
        
        block5_line7_layout = QtWidgets.QHBoxLayout()

        
        #Label Range of cluster
        self.label_351 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_351.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_351.setObjectName("label_351")
        block5_line7_layout.addWidget(self.label_351)

        self.spinBox_52 = QtWidgets.QSpinBox(self.SNF_NEMO) #OLD 48
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.spinBox_52.setProperty("value", 8)
        self.spinBox_52.setObjectName("spinBox_52")
        block5_line7_layout.addWidget(self.spinBox_52)
        
        #Label Variable heatmap
        self.label_352 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_352.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_352.setObjectName("label_352")
        block5_line7_layout.addWidget(self.label_352)
        
        self.spinBox_53 = QtWidgets.QSpinBox(self.SNF_NEMO) #OLD 48
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.spinBox_53.setProperty("value", 20)
        self.spinBox_53.setObjectName("spinBox_53")
        block5_line7_layout.addWidget(self.spinBox_53)
        
        
        
        #Label Variable MAX SNF/NEMO
        self.label_353 = QtWidgets.QLabel(self.SNF_NEMO)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.label_353.setStyleSheet("QWidget{\n"
"\n"
"background-color:rgb(114, 159, 207)\n"
"\n"
"}")
        self.label_353.setObjectName("label_353")
        block5_line7_layout.addWidget(self.label_353)
        
        self.spinBox_54 = QtWidgets.QSpinBox(self.SNF_NEMO) #OLD 48
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.spinBox_54.setMinimum(1)           # Minimum value
        self.spinBox_54.setMaximum(10000)       # You can adjust this upper limit
        self.spinBox_54.setProperty("value", 5000) # Default value
        self.spinBox_54.setObjectName("spinBox_54")
        block5_line7_layout.addWidget(self.spinBox_54)
        
        self.label_351.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.spinBox_52.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_352.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.spinBox_53.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.label_353.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self.spinBox_54.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)

        
        block5_layout.addLayout(block5_line7_layout)
        ...
        self.tabWidget.addTab(self.SNF_NEMO, "")
        
        
        #SNF/NEMO ADVANCE OPTIONS
        self.listWidget_1.addItems(column_names)
        self.listWidget_2.addItems(column_names)
        self.comboBox_54.addItems(column_names)
        
        # Try to set "CONDITION" as the default selected item
        if "CONDITION" in column_names:
            index = column_names.index("CONDITION")
            self.comboBox_54.setCurrentIndex(index)
        ...
        block6_layout = QtWidgets.QVBoxLayout()
        
        main_layout = QtWidgets.QVBoxLayout(self.tab2)
        
        
        block6_line1_layout = QtWidgets.QHBoxLayout() 
...
"}")
        ...
        block6_line1_layout.addWidget(self.pushButton_6)  
        
        block6_layout.addLayout(block6_line1_layout)  
        
        main_layout.addLayout(block6_layout)
    ...
    def retranslateUi2(self, BiomiX_advanced):
        ...
        self.push_design_diablo.setText(_translate("BiomiX", "Upload"))
        ...
        self.label_314.setText(_translate("BiomiX_advanced", "Computational resources / General options Integration "))
        ...
        self.label_243b.setText(_translate("BiomiX_advanced", " N°input features integration"))
        ...
        self.label_345.setText(_translate("BiomiX_advanced", "  SNF-NEMO OPTIONS"))
        ...
        self.comboBox_62_2.setItemText(0, _translate("BiomiX_advanced", "YES"))
        self.comboBox_62_2.setItemText(1, _translate("BiomiX_advanced", "NO"))
        self.comboBox_62_2.setCurrentText('NO')
        ...
        self.label_250_2.setText(_translate("BiomiX_advanced", "Diablo feature selection"))
        ...
        self.label_252_2.setText(_translate("BiomiX_advanced", "N° iteration DIABLO"))
        ...
        self.label_249_2.setText(_translate("BiomiX_advanced", "Design Matrix"))
        ...
        self.label_312_2.setText(_translate("BiomiX_advanced", "DIABLO Options  "))
        ...
        self.label_315.setText(_translate("BiomiX_advanced", "MOFA/DIABLO Factors interpretation "))
        self.label_319.setText(_translate("BiomiX_advanced", "N° Neighbors KNN"))
        self.label_320.setText(_translate("BiomiX_advanced", "Variance Affinity Matrix "))
        self.label_328.setText(_translate("BiomiX_advanced", "Variance Affinity Matrix "))

        self.label_321.setText(_translate("BiomiX_advanced", "N° iteration "))
        self.label_322.setText(_translate("BiomiX_advanced", "N° iteration "))
        self.label_346.setText(_translate("BiomiX_advanced", "NEMO Options"))
        self.label_347.setText(_translate("BiomiX_advanced", "SNF/NEMO Common Options"))
        self.label_348.setText(_translate("BiomiX_advanced", "Enrichment variable "))
        self.label_349.setText(_translate("BiomiX_advanced", "Survival variable "))
        self.label_350.setText(_translate("BiomiX_advanced", "Ground Truth variable "))
        self.label_351.setText(_translate("BiomiX_advanced", "Max clusters expected "))
        self.label_352.setText(_translate("BiomiX_advanced", "N° feature heatmap "))
        self.label_353.setText(_translate("BiomiX_advanced", "N° feature SNF/NEMO "))
        ...
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab), _translate("BiomiX_advanced", "MOFA/DIABLO"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.SNF_NEMO), _translate("BiomiX_advanced", "SNF/NEMO"))
...
#FIRST INPUTS
    #Setting up R version for BiomiX
    script_dir = os.path.dirname(os.path.abspath(__file__))
    rscript_path = get_rscript_path(script_dir)
