...
#
# Invocation (Docker mode, via runner_NEMO in BiomiX_BETA.r):
#   Rscript NEMO_int.R <DIRECTORY> <CELL_TYPE> <GROUP_1> <GROUP_2> <SHARED_DIR> <ITERATIONS>
#
...
if (!exists("Cell_type")) {
  ...
  if (length(cli_args) < 6) {
    stop("Usage: Rscript NEMO_int.R <DIRECTORY> <CELL_TYPE> <GROUP_1> <GROUP_2> <SHARED_DIR> <ITERATIONS>")
  }
...
K.nemo <- as.numeric(COMMAND_ADVANCED$ADVANCED_OPTION_NEMO_OPTIONS[1])
...
only_common_samples = as.logical(COMMAND_ADVANCED$ADVANCED_OPTION_NEMO_OPTIONS[3])

top_feat <- as.numeric(COMMAND_ADVANCED$ADVANCED_OPTION_SNF_NEMO_NUMERIC_OPTIONS[2])
only_common_samples <- as.logical(COMMAND_ADVANCED$ADVANCED_OPTION_SNF_NEMO_NUMERIC_OPTIONS[3])
