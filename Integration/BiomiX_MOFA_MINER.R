...
files2<- files2[grep("\\Metabolomics|\\Methylomics|\\Transcriptomics", files2)]
...
files<- files[grep("\\Metabolomics|\\Methylomics|\\Transcriptomics", files)]
nam<-strsplit(files, "\\Metabolomics|\\Methylomics|\\Transcriptomics")
nam<-unlist(nam)
...
for (omik in (nam)){
        ...
        dir.create(path = paste(directory2,"/Factor_Pathways",sep="") ,  showWarnings = TRUE, recursive = TRUE, mode = "0777")
        pdf(file= paste("Factor_Pathways/", omik, "factor_", numb,".pdf", sep =""), width = 20, height = 9)
        directory_path <-  paste(directory2,"/Factor_Pathways",sep="")
        ...
        write.table(result@result, paste("Factor_Pathways/TABLES/", omik, "factor_", numb, "_positive_HMDB_table_results.tsv", sep =""),quote = FALSE, row.names = F, sep = "\t")        
        ...
        write.table(result@result, paste("Factor_Pathways/TABLES/", omik, "factor_", numb, "_positive_KEGG_table_results.tsv", sep =""),quote = FALSE, row.names = F, sep = "\t")
        ...
        if (length(result) != 0){
                ...
                write.table(result@result, paste("Factor_Pathways/TABLES/", omik, "factor_", numb, "_negative_HMDB_table_results.tsv", sep =""),quote = FALSE, row.names = F, sep = "\t")
                getwd()                
        }
        ...
        if (length(result) != 0){
                ...
                write.table(result@result, paste("Factor_Pathways/TABLES/", omik, "factor_", numb, "_negative_KEGG_table_results.tsv", sep =""),quote = FALSE, row.names = F, sep = "\t")
