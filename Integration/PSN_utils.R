...
plot_imp_heatmap <- function(data, feat_imp, pred.clust, gt.clust=NULL, top_feat=10, 
    ...
    #   levels(pred.clust)
    # )
    #Extention palette to avoid error fro no enough colors per cluster
    pred_cols <- setNames(
      colorRampPalette(brewer.pal(9, "Set1"))(length(levels(pred.clust))),
      levels(pred.clust)
    )
