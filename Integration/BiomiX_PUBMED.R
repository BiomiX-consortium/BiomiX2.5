...
patterns_to_check <- c( "Transcriptomics","Metabolomics", "Methylomics")
...
files<- files[grep("\\Metabolomics|\\Methylomics|\\Undefined|\\Transcriptomics", files)]
nam<-strsplit(files, "\\Metabolomics|\\Methylomics|\\Undefined|\\Transcriptomics")
nam<-unlist(nam)
...
files<- files[grep("\\Metabolomics|\\Transcriptomics", files)]
nam<-strsplit(files, "\\Metabolomics|\\Methylomics|\\Transcriptomics|\\Undefined")
nam<-unlist(nam)
nam<-unique(nam[-grep("*factor*", nam)])
nam <- nam[nam %in% paste(COMMAND$LABEL, "_", sep="")]
...
files<- files[grep("\\Methylomics|\\Transcriptomics", files)]
nam<-strsplit(files, "\\Metabolomics|\\Methylomics|\\Transcriptomics|\\Undefined")
nam<-unlist(nam)
nam<-unique(nam[-grep("*factor*", nam)])
nam <- nam[nam %in% paste(COMMAND$LABEL, "_", sep="")]
...
files <- grep(paste("*factor_", numb, sep=""),list.files(directory2),value=TRUE)
# Patterns to check for
patterns_to_check <- c( "Transcriptomics","Metabolomics", "Methylomics", "Undefined")
# Check if any of the patterns is present in each file name
patterns_present <- sapply(patterns_to_check, function(pattern) any(grepl(pattern, files)))

if (all(patterns_present == c(TRUE, FALSE,FALSE, TRUE))){
        files<- files[grep("|\\Undefined|\\Transcriptomics", files)]
        nam<-strsplit(files, "\\Metabolomics|\\Methylomics|\\Undefined|\\Transcriptomics")
        nam<-unlist(nam)
        nam<-unique(nam[-grep("*factor*", nam)])
        nam <- nam[nam %in% paste(COMMAND$LABEL, "_", sep="")]
...
if (all(patterns_present == c(FALSE, TRUE, FALSE, TRUE))){
...
if (all(patterns_present == c(FALSE, FALSE, TRUE, TRUE))){
...
files <- grep(paste("*factor_", numb, sep=""),list.files(directory2),value=TRUE)
files<- files[grep("|\\Transcriptomics", files)]
nam<-strsplit(files, "\\Metabolomics|\\Methylomics|\\Transcriptomics|\\Undefined")
nam<-unlist(nam)
nam<-unique(nam[-grep("*factor*", nam)])
nam <- nam[nam %in% paste(COMMAND$LABEL, "_", sep="")]
